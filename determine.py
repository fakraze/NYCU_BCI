...
DIFF_ZERO_MAX = 200 # 眨雙眼時 diff 接近 0 的容忍範圍
...
# ========== 極端值判斷設定 ==========
N_POINTS_WINDOW = 100 # 用來計算極端值的點數
...
def main(stream_name=None):
    global LAST_DETECTION_TIME # 宣告使用全域變數
    ...
    # ========== Animation update 函數 ==========
    def update(frame):
        global LAST_DETECTION_TIME # 允許修改 main 函式內的 LAST_DETECTION_TIME
        ...
        # 濾波處理
        try:
            # 使用 try-except 以防 filtfilt 在資料不足時報錯
        ...
        if current_time - LAST_DETECTION_TIME > COOLDOWN_SEC:
            ...
            # --- 修改的部分：計算最近 N_POINTS_WINDOW 的極端值 ---
            ...
            print(f"Extreme Diff: {extreme_diff_for_check:.2f}, "
                  f"Extreme Sum: {extreme_sum_for_check:.2f}, ")
            ...
            # 判斷 左眨眼 (使用者需求：Diff < -500, Sum < -500)
            # 使用絕對值更大的 extreme_sum_for_check 進行判斷
            if extreme_diff_for_check < THRESHOLD_DIFF_NEG and extreme_sum_for_check < THRESHOLD_SUM_NEG:
                # 為了避免偵測到右看的雜訊，確保 Diff 確實是負值
                if extreme_diff_for_check < 0:
                    detected_event = "左眨眼"
                    reported_diff = extreme_diff_for_check
                    reported_sum = extreme_sum_for_check 
                
            # 判斷 右眨眼 (使用者需求：Diff > 500, Sum < -500)
            # 使用絕對值更大的 extreme_sum_for_check 進行判斷
            elif extreme_diff_for_check > THRESHOLD_DIFF_POS and extreme_sum_for_check < THRESHOLD_SUM_NEG:
                # 為了避免偵測到左看的雜訊，確保 Diff 確實是正值
                if extreme_diff_for_check > 0:
                    detected_event = "右眨眼"
                    reported_diff = extreme_diff_for_check
                    reported_sum = extreme_sum_for_check 
                
            # 判斷 眨雙眼 (使用者需求：Sum > 500)
            # 使用絕對值更大的 extreme_sum_for_check 進行判斷
            elif extreme_sum_for_check > 250:
                # 使用 Max(|Diff|) 確保在峰值出現時，Diff 仍接近 0
                if abs(extreme_diff_for_check) < 100: 
                    detected_event = "眨雙眼"
                    # 報告 Max(|Diff|) 和 extreme_sum_for_check
                    reported_diff = extreme_diff_for_check
                    reported_sum = extreme_sum_for_check

            # 如果成功偵測到事件
            if detected_event:
                print(f"[{time.strftime('%H:%M:%S')}] **偵測到事件: {detected_event}** "
    ...
    print("\n*** EOG 事件偵測已啟用，冷卻時間為 1.0 秒 ***")
    print(f"*** 注意: 偵測使用最近 {N_POINTS_WINDOW} 點的絕對值更大的極端值 (Diff 和 Sum 皆是) ***")
    print("偵測條件:")
    print(f"  - 左眨眼: Extreme(diff) < {THRESHOLD_DIFF_NEG} 且 Extreme(sum) < {THRESHOLD_SUM_NEG}")
    print(f"  - 右眨眼: Extreme(diff) > {THRESHOLD_DIFF_POS} 且 Extreme(sum) < {THRESHOLD_SUM_NEG}")
    print(f"  - 眨雙眼: Extreme(sum) > {THRESHOLD_SUM_POS} (且 Max(|diff|) 需接近零，約 < {DIFF_ZERO_MAX})")
